function transtime = semimanual_matching_FcnRUN(omniFilename, sohoFilename) 
%% CME-ICME Matching 
% Written by: Mohamed Nedal 
% omniFilename: OMNI text file. 
% sohoFilename: SOHO excel sheet. 
%% Import OMNI  
diary command_window.txt 
omni_data = load(omniFilename); 
y = omni_data(:,1); % year 
doy = omni_data(:,2); 
...
%% Import SOHO 
soho_data = xlsread(sohoFilename); 
...
transtime = nan(length(C),1); 
for k = 1:length(C) 
    M = C{k}; 
    transtime(k,:) = cme_icme_V4(M, soho(k,8)); 
    fprintf('Event #%d is done ... \n\n', k); 
end 
clear k; 
%% check correlation betn Vsoho vs T 
scatter(soho(:,8), transtime, 'filled'); 
